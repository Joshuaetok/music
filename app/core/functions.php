...
function generateMetaTags($URL) {
    $slug = $URL[1] ?? null;
    $type = $URL[0] ?? null;
    $description = "Welcome to the home of music in Africa!";

    // Define database query based on URL parameter
    if ($type == "song") {
        $query = "SELECT * FROM songs WHERE slug = :slug LIMIT 1";
        $description = "This beautiful song will make your day!";
    } elseif ($type == "blog") {
        $query = "SELECT * FROM blogs WHERE slug = :slug LIMIT 1";
        $description = "Learn more about this beautiful song.";
    }
    ...
    $metaTagsData = [
        ...
        'twitterDomain' => '(link unavailable)',
        'title' => $row['title'],
